# zpower/_trainer.py  —  Trainer v1.2.0
# v1.2 fixes:
#   CRITICAL: actual GradShield state passed to vault (was always "healthy")
...
def _run_fit(trainer_obj, dataset, epochs: int, lr: float, **kwargs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in dataset:
            ...
            # ── v1.2: get REAL grad state (not always "healthy") ──────────
            gs_state  = "healthy"
            stab_info = {}
            if hasattr(model, "_stabilizer") and model._stabilizer:
                ...
                if hasattr(model._stabilizer, "stability_core"):
                    stab_info = model._stabilizer.stability_core.update(
                        loss_val if torch.isfinite(loss) else self._ema_last(model)
                    )
            ...
            # v1.2: pass REAL grad_health to vault
            if hasattr(model, "zp_on_step_end"):
                model.zp_on_step_end({
                    "loss":        loss_val,
                    "grad_health": gs_state,           # real state now
