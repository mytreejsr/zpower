# zpower/stabilize/grad_shield.py  —  GradShield v1.3.0
# v1.3 changes:
#   PERF: Adaptive warmup — uses Welford variance estimate to decide when
#         enough samples exist for reliable adaptive thresholds (replaces
#         fixed _WARMUP_STEPS=20)
#   FIX:  _remove_hooks() now catches RuntimeError for already-removed hooks
#   API:  Added __repr__
...
# v1.3: minimum variance confidence before adaptive thresholds kick in
_MIN_VARIANCE_SAMPLES = 10   # absolute minimum
_VARIANCE_CONFIDENCE  = 0.5  # std/mean ratio must be below this for confidence


class GradShield:
    """
    GradShield v1.3.0 — Adaptive gradient health monitor.

    v1.1: adaptive per-layer thresholds, deque history
    v1.2: O(1) health_rate (running counters), __del__/context manager,
          health() API
    v1.3: Adaptive warmup — exits warmup when per-layer variance is
          confident (not after fixed N steps). Prevents premature
          adaptive thresholds on layers with few gradient observations.
          Also fixes _remove_hooks() for already-removed hooks.
    """
    ...
    def __init__(
        ...
        # O(1) running counters for health_rate
    ...
    def attach_to_model(self, model) -> "GradShield":
        """Attach gradient hooks to model. Safe to call multiple times — removes old hooks first."""
        if not _TORCH:
    ...
    def layer_stats(self) -> Dict[str, Dict]:
        ...
        for name, s in self._layer_stats.items():
            ...
            out[name] = {
                ...
                "samples":       len(s["norms"]),
                "warmup_active": not self._is_warmed_up(name),
            }
        return out
    ...
    def __repr__(self) -> str:
        hr = round(self._window_healthy / max(self._window_total, 1), 3)
        return (f"GradShield(step={self._step}, health_rate={hr}, "
                f"adaptive={self.adaptive}, layers={len(self._layer_stats)})")

    # ── Internal ───────────────────────────────────────────────────────────

    def _is_warmed_up(self, layer_name: str) -> bool:
        """v1.3: Check if a layer has enough confident stats for adaptive thresholds."""
        s = self._layer_stats.get(layer_name, {})
        count = s.get("_count", 0)
        if count < _MIN_VARIANCE_SAMPLES:
            return False
        # Check variance confidence: std/mean ratio
        mean = s.get("mean", 0)
        std  = s.get("std", 0)
        if abs(mean) < 1e-10:
            return count >= _MIN_VARIANCE_SAMPLES * 2
        cv = abs(std / (mean + 1e-10))  # coefficient of variation
        return cv < _VARIANCE_CONFIDENCE or count >= 20

    def _thresholds(self, layer_name, norm):
        if not self.adaptive:
            return self._fixed_clip, self._fixed_vanish, self._fixed_explode
        # v1.3: use confidence-based warmup instead of fixed step count
        if not self._is_warmed_up(layer_name):
            return self._fixed_clip, self._fixed_vanish, self._fixed_explode
        s = self._layer_stats.get(layer_name, {})
    ...
    def _remove_hooks(self):
        """v1.3 fix: catch RuntimeError for already-removed hooks."""
        remaining = []
        for h in self._hooks:
            try:
                ...
                remaining.append(h)  # Unknown error, keep reference
