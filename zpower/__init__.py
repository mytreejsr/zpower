...
class Trainer:
    ...
    auto_heal      : Reserved for v2

    Example
    -------
    >>> trainer = zp.Trainer(model, stabilize=True, weight_vault=True)
...
╚══════════════════════════════════════════════════════╝
  ...
  Core Modules:
    ...
    zp.stabilize             GradShield + StabilityCore
  ...
  Quick start (training from scratch):
    trainer = zp.Trainer(my_model, stabilize=True, weight_vault=True)
    trainer.fit(train_loader, epochs=20)
    print(trainer.weight_report())
