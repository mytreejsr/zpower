# zpower/weights/vault.py  —  WeightVault: Performance-gated weight snapshots
...
class WeightVault:
    """
    WeightVault — Records weight snapshots ONLY when performance is good enough.
    ...
    def load(self, path: str):
        ...
        keys = [k for k in archive.files if not k.endswith(("__score", "__epoch"))]
        ...
        for layer, snap_keys in grouped.items():
            ...
            for sk in sorted(snap_keys):
                ...
                snap  = VaultSnapshot(
                    ...
                    weights=archive[sk], metrics={}, perf_score=score,
                )
